<?php
  ...
  $status_table = "SELECT distinct(diamond_status) AS status,count(diamond_status) AS count,CEIL(sum(diamond_size)) AS carat FROM `diamonds` WHERE diamond_type = 'Fancy' AND diamond_lot_no LIKE 'F%' AND diamond_status NOT IN ('Invoiced','Deleted') group by diamond_status ORDER BY diamond_id DESC";
...
AND `diamond_status` NOT IN ('Invoiced','Deleted')"));
$black_office = mysqli_query($conn,"SELECT office_name,office_id from offices  WHERE office_id IN (SELECT DISTINCT(office_id) FROM diamonds where  diamond_type = 'Fancy' AND diamond_lot_no LIKE 'F%' AND diamond_status NOT IN ('Invoiced','Deleted'))");


$total_count = mysqli_fetch_assoc(mysqli_query($conn,"SELECT count(*) as count_total FROM `diamonds` WHERE diamond_type = 'Fancy' AND diamond_lot_no LIKE 'F%' AND diamond_status NOT IN ('Invoiced','Deleted')"));
$total_carat = mysqli_fetch_assoc(mysqli_query($conn,"SELECT sum(diamond_size) as sum_total FROM `diamonds` WHERE diamond_type = 'Fancy' AND diamond_lot_no LIKE 'F%' AND diamond_status NOT IN ('Invoiced','Deleted')"));
?>
      ...
      <table class="table col-md-4 table-bordered" style="border:1px solid black;">
        ...
        <tbody>
            <?php while($offices = mysqli_fetch_assoc($black_office)):
              $count_office = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(office_id) as count,sum(diamond_size) as carat FROM diamonds where diamond_type = 'Fancy' AND diamond_lot_no LIKE 'F%' AND diamond_status NOT IN ('Invoiced','Deleted') AND office_id = '".$offices['office_id']."'"));
              ?>
...
<div class="row">
      ...
      <fieldset class="form-group">
          <?php while($row_shape = mysqli_fetch_assoc($sql2)): ?>
            <?php
                 $sql_count = mysqli_query($conn, "SELECT COUNT(*) from `diamonds` WHERE diamond_type = 'Fancy' AND `diamond_shape_id` = '".$row_shape['attribute_id']."' AND diamond_lot_no LIKE 'F%' AND diamond_status NOT IN ('Invoiced','Deleted') ");
